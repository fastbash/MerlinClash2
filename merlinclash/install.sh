...
get_fw_type() {
    KS_TAG=$(nvram get extendno|grep -Eo "kool.+")
    if [ -d "/koolshare" ];then
        if [ -n "${KS_TAG}" ];then
            FW_TYPE_CODE="2"
...
platform_test(){
    ...
    if [ -d "/tmp/merlinclash/bin32" ]; then
        ...
        PKG_ARCH="arm64"
    elif [ -d "/tmp/merlinclash/bin380" ]; then
        PKG_ARCH="arm380"
    elif [ -d "/tmp/merlinclash/bin384" ]; then
        PKG_ARCH="arm384"
    else
    ...
    # ARM384
    if [ "${PKG_ARCH}" = "arm384" ]; then
        case $(uname -m) in
            armv7l)
                if uname -o | grep -q Merlin && [ -d "/koolshare" ] && nvram get buildno | grep -q 384 || nvram get buildno | grep -q 386;then
                    echo_date "固件平台【koolshare merlin armv7l 384/386】符合安装要求，开始安装插件！"
                elif [ "$MODEL" = "TUF-AX3000" ] || [ "$MODEL" = "RT-AX82U" ] || [ "$MODEL" = "RT-AX95Q" ] || [ "$MODEL" = "RT-AX56_XD4" ] && [ -d "/koolshare" ];then
                    echo_date "机型：$MODEL $(_get_type) 符合安装要求，开始安装插件！"
                else
                    echo_date "本插件适用于【koolshare merlin armv7l 384/386】固件平台，你的固件平台不能安装！！！"
                    echo_date "退出安装！"
                    exit 1
                fi
                ;;
            *)
                echo_date "本插件适用于【koolshare merlin armv7l 384/386】固件平台，你的平台：$(uname -m)不能安装！！！"
                echo_date "退出安装！"
                exit 1
            ;;
        esac
...
install_now(){
    ...
    if [ "$(df | grep -w "/jffs" | awk '{print $4}')" -gt "30000" ];then
    ...
    bin_doc="/tmp/merlinclash/bin$(echo "$PKG_ARCH" | sed 's#arm##g')"

    #检查是否有jq
    if [ -f /usr/bin/jq ];then
        rm -rf "${bin_doc}"/jq >/dev/null 2>&1
        rm -rf "${bin_doc}"/jq >/dev/null 2>&1
    ...
    if [ -d /tmp/merlinclash/bin32 ]; then
        ...
        if [ "${LINUX_VER}" != "44" ]; then
            rm -rf "${bin_doc}"/haveged
        fi
        mv -f "${bin_doc}"/subconverter /koolshare/merlinclash/subconverter/
    elif [ -d /tmp/merlinclash/bin64 ]; then
        mv -f "${bin_doc}"/subconverter /koolshare/merlinclash/subconverter/
    fi

    if df | grep -wq /data && [ "$(df | grep -w /jffs | awk '{print $4}')" -le "102400" ];then
        echo_date "/jffs 分区空间不足100M，检测到存在/data分区，优先安装到/data分区"
        for bin_file in "${bin_doc}"/* ;do
            if [ "$(df | grep -w /data | awk '{print $4}')" -gt "$(du -s "$bin_file" | awk '{print $1}')" ];then
                echo_date "正在安装 ${bin_file##*/} 到 /data 分区..."
                if [ -f "/data/${bin_file##*/}-MC2" ];then
                    rm -rf "/data/${bin_file##*/}-MC2"
                fi
                cp "$bin_file" "/data/${bin_file##*/}-MC2"
                chmod a+x "/data/${bin_file##*/}-MC2"
                ln -sf "/data/${bin_file##*/}-MC2" "/koolshare/bin/${bin_file##*/}"
            else
                echo_date "/data 分区空间不足，安装 ${bin_file##*/} 到 /koolshare/bin/ "
                cp -rf "$bin_file" /koolshare/bin/
            fi
        done
    else
        cp -rf "${bin_doc}"/* /koolshare/bin/
    fi
    ...
    if [ -f "$Geosite_PATH" ] && [ "$(wc -c < "$Geosite_PATH" 2>/dev/null)" -gt 1000000 ]; then
    ...
    echo_date "数据初始化"
    #
    dbus set merlinclash_mixport_enable=1
    # tcp 并发
    dbus set merlinclash_tcp_concurrent=1
    # 代理路由自身
    # dbus set merlinclash_dnsgoclash=1
    # dbus set merlinclash_cus_routingmark=255
    ...
    # fi
    dbus set merlinclash_queue_switch=0
    #判断是否开启大陆绕行IP
    ...
    #提取默认密码
    secret=$(awk '/secret:/{print $2}' /koolshare/merlinclash/yaml_basic/head.yaml | sed 's/"//g')
    ...
    elif [ "${PKG_ARCH}" = "arm384" ]; then
        dbus set merlinclash_version_local="$CUR_VERSION.A384"
    elif [ "${PKG_ARCH}" = "arm380" ]; then
        dbus set merlinclash_version_local="$CUR_VERSION.A380"
    else
        dbus set merlinclash_version_local="$CUR_VERSION.A??"
    fi
    ...
    #设置内核版本
    ret=$(env -i PATH=${PATH} /koolshare/bin/clash -v 2>/dev/null | head -n 1)
    clashTmpV1=$(echo "$ret" | cut -d " " -f2)
    clashTmpV2=$(echo "$ret" | cut -d " " -f3)
    ...
    if [ "$(find /koolshare/merlinclash/yaml_bak/*.yaml | wc -l)" -gt 0 ]; then
    ...
    if [ ! -f /koolshare/merlinclash/yaml_bak/yamlscus.txt ]; then
        touch /koolshare/merlinclash/yaml_bak/yamlscus.txt
    fi
    if [ ! -L /tmp/upload/yamlscusyamlscuslist.txt ]; then
    ...
    if [ ! -f /koolshare/merlinclash/yaml_bak/yamlscuslist.txt ]; then
        touch /koolshare/merlinclash/yaml_bak/yamlscuslist.txt
    fi
    if [ ! -L /tmp/upload/yamlscuslist.txt ]; then
