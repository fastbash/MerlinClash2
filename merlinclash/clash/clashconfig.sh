...
#提取配置认证码
secret=$(awk '/secret:/{print $2}' $yamlpath | xargs echo -n)

#20200904 新增host.yaml处理
hostsel=$(get merlinclash_hostsel)
hostsyaml=/koolshare/merlinclash/yaml_basic/host/$hostsel.yaml
tproxy=$(awk '/tproxy:/{print $2}' $yamlpath | xargs echo -n)
#端口取值
    httpport=$(awk -F: '/^port/{print $2}' $yamlpath | xargs echo -n)
    socksport=$(awk -F: '/^socks-port/{print $2}' $yamlpath | xargs echo -n)
    proxy_port=$(awk -F: '/^redir-port/{print $2}' $yamlpath | xargs echo -n)
    tproxy_port=$(awk -F: '/^tproxy-port/{print $2}' $yamlpath | xargs echo -n)
    dnslistenport=$(awk -F: '/listen/{print $3}' $yamlpath | xargs echo -n)

if [ "$modesel" = "default" ]; then
    modesel=$(grep "^mode:" $yamlpath | awk -F "[: ]" '{print $3}'| xargs echo -n)
fi
...
check_coremark(){
    ...
    cur_patition=$(df -h | /bin/grep /jffs | awk '{print $1}')
    jffs_device="not mount"
    if [ -n "${cur_patition}" ];then
          jffs_device=${cur_patition}
    fi
    mounted_nu=$(mount | /bin/grep "${jffs_device}" | grep -E "/tmp/mnt/|/jffs"|/bin/grep -c "/dev/s")
...
detect_ip(){    
    ...
    ckStep4="$(echo $1 | grep -Ec $regex_v4)"
    ckStep6="$(echo $1 | grep -Ec $regex_v6)"
...
move_config(){
    ...
    if [ -f "/tmp/upload/${uploadfilename:?}" ]; then
        ...
        if [ $? = "1" ]; then
            ...
            cp -rf "/tmp/upload/${uploadfilename:?}" /tmp/upload/yaml/
...
write_setmark_cron_job(){
    if [ "${coremark}" = "1" ]; then
      ...
      if [ "$mcenable" = "1" ];then
          if [ -n "$(pidof clash)" ] && [ "$(netstat -anp | grep clash | head -n 5)" ] && [ -z "$(grep "Parse config error" /tmp/clash_run.log | head -n 5)" ] ; then
...
kill_cron_job() {
    ...
    if cru l | grep -q httpd_watchdog; then
    ...
    if cru l | grep -q clash_prenetflix; then
...
kill_setmark(){
    pid_setmark=$(ps | grep -v grep | grep clash_node_mark.sh | awk '{print $1}')
...
close_in_five() {
    ...
    sed -i '/dhcp-option-force=br1/d' /etc/dnsmasq.conf
    sed -i '/dhcp-option-force=br2/d' /etc/dnsmasq.conf
...
lan_bypass(){    
    ...
    wlnum=0
    if lsmod | grep -q ip_set_hash_mac; then
        ...
        if [ "$list_flag" = "2" ]; then
            if [ -n "$nokpacl_nu" ]; then
                for nokpacl in $nokpacl_nu; do
                    ...
                    proxy_name=$(eval echo \$merlinclash_nokpacl_name_$nokpacl)
                    if [ "$mnm" = "1" ];then
                        echo_date "设备IP地址：【$ipaddr】，MAC地址：【$macaddr】，端口：【$ports】，代理模式：【$(get_mode_name $proxy_mode)】" >> $LOG_FILE
                    fi
                    if [ "$mnm" = "2" ];then
                        macaddr=""
                    ...
                    if [ "$mnm" = "3" ];then
                        ipaddr=""
                        echo_date "设备MAC地址：【$macaddr】，端口：【$ports】，代理模式：【$(get_mode_name $proxy_mode)】" >> $LOG_FILE
                    fi
                    if [ "$mnm" = "3" ] && [ "$macaddr" = "" ]; then
                        echo_date "设备$proxy_name MAC地址为空，不做处理，跳过。" >> $LOG_FILE
                        continue
...
creat_ipset() {
    ...
    if ! lsmod | grep -q ip_set_bitmap_port && [ -f "/lib/modules/4.1.27/kernel/net/netfilter/ipset/ip_set_bitmap_port.ko" ];then
...
load_tproxy() {
    ...
    checkmoduleisloaded() {
        if lsmod | grep -q $MODULE; then return 0; else return 1; fi
    }

    for MODULE in $MODULES; do
        if ! checkmoduleisloaded; then
            #insmod /lib/modules/${OS}/kernel/net/netfilter/${MODULE}.ko
            if [ "${LINUX_VER}" -eq "419" ] || [ "${LINUX_VER}" -eq "54" ];then
                modprobe ${MODULE}.ko
            else
                insmod /lib/modules/${OS}/kernel/net/netfilter/${MODULE}.ko
            fi
...
apply_nat_rules3() {
    ...
    dem2=$(grep "enhanced-mode:" "$yamlpath" | awk -F "[: ]" '{print $5}')
        ...
        if [ "$list_flag" = "1" ]; then
            if [ -n "$nokpacl_nu" ]; then
                for nokpacl in $nokpacl_nu; do
                    ...
                    [ "$mnm" = "2" ] && macaddr="" && echo_date "【方案二】设备IP地址：【$ipaddr】，端口：【$ports】，代理模式：【$(get_mode_name $proxy_mode)】" >> $LOG_FILE
                    [ "$mnm" = "3" ] && ipaddr="" && echo_date "【方案二】设备MAC地址：【$macaddr】，端口：【$ports】，代理模式：【$(get_mode_name $proxy_mode)】" >> $LOG_FILE
                    if [ "$mnm" = "3" ] && [ "$macaddr" = "" ]; then
...
start_clash(){
    ...
    count1=$(grep -n "^proxies:" $yamlpath | awk -F ":" '{print $1}')
    count2=$(grep -n "^proxy-groups:" $yamlpath | awk -F ":" '{print $1}')
...
check_yaml(){
    ...
    if [ -f "$yamlpath" ]; then
        ...
        #提取配置监听端口
        ecport=$(awk -F: '/external-controller/{print $3} ' $yamlpath | xargs echo -n)
        ecip=$(awk -F: '/external-controller/{print $2} ' $yamlpath | xargs echo -n)
    else
...
start_custom(){
  ...
  #内核代理组状态记忆&fake-ip缓存启用
    ...
    routing_mark=$(awk '/routing-mark:/{print $2}' $yamlpath | xargs echo -n)
    sed -i "s/^routing-mark: \"255\"/routing-mark: \"$mds\"/g" $yamlpath
    echo_date "设置路由流量标记值(Routing-Mark)为：$mcrm" >> $LOG_FILE

    #端口取值
    port=$(awk -F: '/^port/{print $2}' $yamlpath | xargs echo -n)
    socksport=$(awk -F: '/^socks-port/{print $2}' $yamlpath | xargs echo -n)
    proxy_port=$(awk -F: '/^redir-port/{print $2}' $yamlpath | xargs echo -n)
    tproxy_port=$(awk -F: '/^tproxy-port/{print $2}' $yamlpath | xargs echo -n)
    ecport=$(awk -F: '/external-controller/{print $3}' $yamlpath | xargs echo -n)
...
check_dnsplan(){
    ...
    case $dnsplan in
    rh)
        dns_yaml_file="/koolshare/merlinclash/yaml_dns/redirhost.yaml"
        #默认方案
        ...
        if [ -n "$localDNS" ];then
            sed -i "s#    - 223.5.5.5#${localDNS}#g" "$dns_yaml_file"
        fi
        ;;
    fi)
        dns_yaml_file="/koolshare/merlinclash/yaml_dns/fakeip.yaml"
    ...
    # # 修改dns监听地址为LANIP，增强安全性
    # LANIP=$(nvram get lan_ipaddr)
    # if echo "$LANIP" | grep -qE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+';then
    #     sed -i "s/192.168.50.1/$LANIP/g" "$dns_yaml_file"
    # else
        sed -i "s/192.168.50.1//g" "$dns_yaml_file"
    # fi
    cat "$dns_yaml_file" >> $yamlpath
