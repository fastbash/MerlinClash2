...
write_dictionary(){
	if [ "$subscription_type" == "4" ]; then #HND_SC订阅 
		/bin/sh /koolshare/scripts/clash_dictionary.sh "$upname" "$subscription_type" "$merlinc_link" "$clashtarget" "$acl4ssrsel" "$emoji" "$udp" "$appendtype" "$sort" "$fnd" "$include" "$exclude" "$scv" "$tfo" "none" "$xudp"
	elif [ "$subscription_type" == "5" ]; then #384_ACL订阅 
		/bin/sh /koolshare/scripts/clash_dictionary.sh "$upname" "$subscription_type" "$merlinc_link" "$clashtarget" "$acl4ssrsel" "$emoji" "$udp" "$appendtype" "$sort" "$fnd" "$include" "$exclude" "$scv" "$tfo" "$addr" "$xudp"
	elif [ "$subscription_type" == "6" ]; then #HND_自定订阅 
		/bin/sh /koolshare/scripts/clash_dictionary.sh "$upname" "$subscription_type" "$merlinc_link" "$clashtarget" "$acl4ssrsel" "$emoji" "$udp" "$appendtype" "$sort" "$fnd" "$include" "$exclude" "$scv" "$tfo" "none" "$xudp"
	elif [ "$subscription_type" == "7" ]; then #HND_远程订阅 
		echo_date "远程配置地址是：$urlinilink" >> $LOG_FILE
		/bin/sh /koolshare/scripts/clash_dictionary.sh "$upname" "$subscription_type" "$merlinc_link" "$clashtarget" "none" "$emoji" "$udp" "$appendtype" "$sort" "$fnd" "$include" "$exclude" "$scv" "$tfo" "none" "$xudp" "$urlinilink"
	else # "8"=384_远程订阅 
		/bin/sh /koolshare/scripts/clash_dictionary.sh "$upname" "$subscription_type" "$merlinc_link" "$clashtarget" "none" "$emoji" "$udp" "$appendtype" "$sort" "$fnd" "$include" "$exclude" "$scv" "$tfo" "$addr" "$xudp" "$urlinilink"
	fi
}
